...
- (QDRLoginView *)loginView
{
    if (!_loginView) {
        ...
        [_loginView addObserver:self forKeyPath:@"gobackKVO" options:NSKeyValueObservingOptionOld|NSKeyValueObservingOptionNew context:nil];
...
- (void)dealloc
{
    ...
    [self.loginView removeObserver:self forKeyPath:@"gobackKVO" context:nil];
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    if (object == self.loginView) {
        if([keyPath isEqualToString:@"KVOForgotNum"])
        {
            ...
            SJPhoneViewController *vc = [[SJPhoneViewController alloc] initWithTitleStr:@"重置密码" titleStr:@"请输入您的账号"];
            ...
            // 响应变化处理 注册
            SJPhoneViewController *vc = [[SJPhoneViewController alloc] initWithTitleStr:@"手机注册" titleStr:@"请输入您的手机号"];
            [self.navigationController pushViewController:vc animated:YES];
            ...
            if ([WXApi isWXAppInstalled]) {
                ...
                // 微信登录事件
                DLog(@"微信登录跳转");
                DLog(@"%d", [WXApi sendAuthReq:req viewController:self delegate:appDelegate]);
            }
        }else if ([keyPath isEqualToString:@"gobackKVO"]){
...
-(void)requestUserInfoByToken:(NSString *)token andOpenid:(NSString *)openID{
    ...
    [manager GET:[NSString stringWithFormat:@"https://api.weixin.qq.com/sns/userinfo?access_token=%@&openid=%@",token,openID] parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        SJWXLgoinModel *qqModel  = [SJWXLgoinModel mj_objectWithKeyValues:dic];
...
- (void)getUserInfoResponse:(APIResponse*) response{
    ...
    NSLog(@" response %@",dic);
    SJQQLgoinModel *qqloginModel = [SJQQLgoinModel mj_objectWithKeyValues:dic];
